...
<!-- Builds DC metadata file, and writes to directory named by PQDT file number -->
        <xsl:result-document href="{marc:datafield[@tag=871]/marc:subfield[@code='c']}/dublin_core.xml" encoding="UTF-8" format="xml">
                <dublin_core>
                    <!-- author-->
...
<!-- Builds contents text file for DSpace batch ingest process, and writes to same directory as above -->        
        <xsl:result-document href="{marc:datafield[@tag=871]/marc:subfield[@code='c']}/contents" encoding="UTF-8" format="text">
            <xsl:value-of select="marc:datafield[@tag=871]/marc:subfield[@code='c']"/><xsl:text>.pdf&#09;bundle:ORIGINAL</xsl:text>
